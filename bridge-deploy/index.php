<?php
/**
 * Rolplay – Apotex Analytics Bridge v3.0
 * Single-file PHP bridge — reads directly from rolplay_apotex_robin
 * DB: 138.68.248.149 | User: apotexDashboard (SELECT + TEMP TABLES)
 *
 * ACTIONS
 *   ping
...
// ── Config — DB1: rolplay_apotex_robin (platform) ─────────
define('DB_HOST','138.68.248.149');
define('DB_PORT',3306);
define('DB_NAME','rolplay_apotex_robin');
define('DB_USER','apotexDashboard');
define('DB_PASS','Ecosphere-Durably-Pettiness-Quartet');
define('DB_COLL','utf8mb4');
define('BRIDGE_START', microtime(true));

// ── Config — DB2: roleplay_demorp6 (Set 3 exercises) ──────
// Same MySQL host as simulador_Asistentes (137.184.208.240).
// Override credentials in .env: DB2_HOST, DB2_USER, DB2_PASS, DB2_NAME
define('DB2_HOST','137.184.208.240');
define('DB2_PORT',3306);
define('DB2_NAME','roleplay_demorp6');
define('DB2_USER','apotexDashboard');   // update in .env if credentials differ
define('DB2_PASS','');                  // set via .env: DB2_PASS=<password>

// Override from .env if present
foreach(@file(__DIR__.'/.env',FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES)?:[] as $ln){
    if(!$ln||$ln[0]==='#') continue;
    $p=strpos($ln,'='); if($p===false) continue;
...
case 'sim.demorp6':
    ...
    if (empty($ids)) err('id[] required — pass ?id=470&id=471 …');
    ...
    out(['ok' => true, 'data' => $sims, 'total_records' => count($sims)]);
