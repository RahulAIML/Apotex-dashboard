...
export function AIAssistant() {
  ...
  const bridgeOverview = useBridgeOverview()
  ...
  function buildContext(): string {
    ...
    if (isRolplayPage && bridgeOverview.data) {
      const ov = bridgeOverview.data
      ctx +=
        `APOTEX BRIDGE DATA — ${pageName}\n` +
        `Sessions: ${ov.total_sessions} | Avg Score: ${ov.avg_score}% | ` +
        `Pass Rate: ${ov.pass_rate_pct}% | Active Users: ${ov.unique_users} | ` +
        `Members: ${ov.total_members} | Logins: ${ov.total_logins}\n\n`
    }
  ...
  const handleSend = async () => {
    ...
    const fullPrompt =
      buildContext() +
      imageInstruction +
      `\n\nYou are the Apotex Analytics AI assistant. Respond in ${lang}. ` +
  ...
  const greeting = language === 'es'
    ? `¡Hola! Soy el asistente IA de Apotex. Estás en **${pageName}**. Puedo analizar el rendimiento del equipo, identificar áreas de mejora y generar perspectivas. También puedes **pegar o adjuntar imágenes** del dashboard para análisis visual. ¿En qué te puedo ayudar?`
    : `Hello! I'm the Apotex AI assistant. You're viewing **${pageName}**. I can analyze team performance, identify improvement areas, and generate data-driven insights. You can also **paste or attach dashboard screenshots** for visual analysis. How can I help?`
