// Serves dist/ as a static SPA, proxies /apotex/api/* and /bridge/* to the upstream
// API behind an auth check, and handles authentication
...
const DASHBOARD_NAME = process.env.DASHBOARD_NAME ?? 'Apotex'
...
const PROXIES = [
  ...
  {
    // /bridge?action=X → /apotex/bridge/?action=X
    prefix: '/bridge',
    target: 'https://serv.aux-rolplay.com',
    rewrite: (p) => `/apotex/bridge/${p.slice('/bridge'.length).replace(/^\//, '')}`,
  },
]
